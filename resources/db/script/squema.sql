...
CREATE TABLE planet(
    ...
    description text,
    planet_prerequisite_id bigint REFERENCES planet(id),
    CONSTRAINT pk_planet PRIMARY KEY (id),
    CONSTRAINT uk_planet_name UNIQUE("name"),
    CONSTRAINT uk_planet_planet_prerequisite UNIQUE(planet_prerequisite_id)
);
...
CREATE TABLE mission (
    ...
    CONSTRAINT uk_mission_mission_prerequisite UNIQUE(mission_prerequisite_id),
    CONSTRAINT fk_mission_planet FOREIGN KEY(planet_id) REFERENCES planet(id)
);
...
CREATE TABLE user_mission (
    ...
    CONSTRAINT ck_user_mission_status CHECK (status IN ('IN_PROGRESS', 'COMPLETED', 'BLOCKED'))
);
...
CREATE TABLE achievement (
    ...
    CONSTRAINT uk_achievement_level UNIQUE(level_id),
    ...
    CONSTRAINT fk_achievement_level FOREIGN KEY (level_id) REFERENCES "level"(id) ON DELETE CASCADE,
    
    CONSTRAINT ck_achievement_bonus_xp CHECK (bonus_xp >= 0),
    CONSTRAINT ck_achievement_type CHECK (
        ("type" = 'LEVEL' AND level_id IS NOT NULL AND mission_id IS NULL AND planet_id IS NULL) OR
        ("type" = 'MISSION' AND mission_id IS NOT NULL AND level_id IS NULL AND planet_id IS NULL) OR
        ("type" = 'PLANET' AND planet_id IS NOT NULL AND level_id IS NULL AND mission_id IS NULL)
    )
);
